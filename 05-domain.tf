...
resource "linode_domain" "siwko_test_org" {
    type = "master"
    domain = "siwko-test.org"
    soa_email = "asiwko@siwko.org"
}
...
resource "linode_domain_record" "aws01_siwko_test_org" {
    domain_id = linode_domain.siwko_test_org.id
    name = "aws01"
    record_type = "A"
    target = aws_instance.test_virtual_machine_01.public_ip
}

resource "linode_domain_record" "aws02_siwko_test_org" {
    domain_id = linode_domain.siwko_test_org.id
    name = "aws02"
    record_type = "A"
    target = aws_instance.test_virtual_machine_02.public_ip
