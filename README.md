...
## Overview
This experiment uses Terraform to create a single virtual machine on [Amazon's AWS](https://aws.amazon.com/).  It is part of a series of experiments begun in early 2026.  I have structured the Terraform files with sequence numbers to show the logical flow of resource creation.  Roughly the sequence is:
...
* I had an issue with the default VPC.  If I did not define a unique VPC, Terraform would try to destroy the default every time it destroyed and rebuilt the instance.  This hung the Terraform job.
